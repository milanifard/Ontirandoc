<?php 
/*
 صفحه  کلاس مدیریت امنیت مربوط به : الگوی جلسات
	برنامه نویس: امید میلانی فرد
	تاریخ ایجاد: 89-2-30
*/
class security_SessionTypes
{
	...
	function SaveFieldPermission($RecID, $FieldName, $SelectedPersonID, $AccessType)
	{
		...
		$query = "insert into sessionmanagement.PersonPermissionsOnFields (TableName, PersonID, RecID, FieldName, AccessType) values ('SessionTypes', ?, ?, ?, ?)";
		$mysql->Prepare($query);
	...
	{
		$mysql = pdodb::getInstance();
		$query = "insert into sessionmanagement.PersonPermissionsOnTable (TableName, PersonID, RecID, DetailTableName, AddAccessType, RemoveAccessType, UpdateAccessType, ViewAccessType) values ('SessionTypes', ?, ?, ?, ?, ?, ?, ?)";
		$mysql->Prepare($query);
...
// متدی که دسترسی برای یک فیلد روی یک رکورد را برای یک کاربر برمی گرداند
	...
	{
		$mysql = pdodb::getInstance();
		$query = "select AccessType from sessionmanagement.PersonPermissionsOnFields where TableName='SessionTypes' and PersonID=? and RecID=? and FieldName=?";
		$mysql->Prepare($query);
...
// AccessType: Add, Update, Remove, View
	...
	{
		$mysql = pdodb::getInstance();
		$query = "select ".$AccessType."AccessType from sessionmanagement.PersonPermissionsOnTable where TableName='SessionTypes' and PersonID=? and RecID=? and DetailTableName=?";
		$mysql->Prepare($query);
...
// متدی که تنظیمات دسترسی کاربر به یک رکورد را حذف می کند
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sessionmanagement.PersonPermissionsOnFields where TableName='SessionTypes' and PersonID=? and RecID=?";
		$mysql->Prepare($query);
...
// متدی که تنظیمات دسترسی کاربر به جداول جزییات یک رکورد را حذف می کند
	...
	{
		$mysql = pdodb::getInstance();
		$query = "delete from sessionmanagement.PersonPermissionsOnTable where TableName='SessionTypes' and PersonID=? and RecID=?";
		$mysql->Prepare($query);
		$mysql->ExecuteStatement(array($SelectedPersonID, $RecID));
	}
	...
	static function LoadUserPermissions($PersonID, $RecID)
	{
		...
		$query = "select AccessType, FieldName from sessionmanagement.PersonPermissionsOnFields where TableName='SessionTypes' and PersonID=? and RecID=?";
		$mysql->Prepare($query);
		...
		$query = "select * from sessionmanagement.PersonPermissionsOnTable where TableName='SessionTypes' and PersonID=? and RecID=?";
		$mysql->Prepare($query);
		$res = $mysql->ExecuteStatement(array($PersonID, $RecID));
		while($rec = $res->fetch())
		{
