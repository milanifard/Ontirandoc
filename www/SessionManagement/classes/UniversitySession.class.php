...
class manage_UniversitySessions
{
	...
	*/
	static function Search($CurrentUser, $SessionTypeID, $SessionNumber, $SessionTitle, $SessionFromDate, $SessionToDate, $SessionLocation, $PreCommandKeyWord, $DecisionKeyWord, $OtherConditions, $FromRec = 0, $NumberOfRec = 30, $OrderByFieldName="", $OrderType="")
	{
		...
		$query = "select AccessType, UniversitySessions.UniversitySessionID
			...
			, UniversitySessions.DescisionsFileStatus
			, UniversitySessions.DescisionsListStatus
			, s1.SessionTypeTitle  as s1_SessionTypeTitle 
			, g2j(SessionDate) as SessionDate_Shamsi 
			, CASE UniversitySessions.SessionStatus 
				WHEN '0' THEN 'درخواست تشکیل' 
	...
	function GetSearchResultCount($CurrentUser, $SessionTypeID, $SessionNumber, $SessionTitle, $SessionFromDate, $SessionToDate, $SessionLocation, $PreCommandKeyWord, $DecisionKeyWord, $OtherConditions)
	{
		$mysql = pdodb::getInstance();
		$k=0;
		$ret = array();
	...
	* @return کد جلسه اضافه شده	*/
	...
	function SignTheDescesionFile($CurrentUser, $UniversitySessionID, $SignDescription)
	{
		$mysql = pdodb::getInstance();
		$query = "update sessionmanagement.SessionMembers set SignTime=now(), SignDescription=? , SignStatus='YES' where UniversitySessionID=? and MemberPersonID=?";
		$mysql->Prepare($query);
		$mysql->ExecuteStatement(array($SignDescription, $UniversitySessionID, $CurrentUser));
