...
$SelectOptions = "";
$list = manage_FormsStruct::GetList(" FormsStructID=40 ");
...
if(isset($_REQUEST["FormsRecordsID"]))
{
	$query = "select StepTitle, FormsFlowSteps.FormsStructID, RelatedRecordID, FormsRecords.FormFlowStepID, SenderID, g2j(SendDate) as gSendDate
					...
					JOIN hrms_total.persons as p1 on (FormsRecords.SenderID=p1.PersonID) 
					JOIN hrms_total.persons as p2 on (FormsRecords.CreatorID=p2.PersonID)
...
if(isset($_REQUEST["SelectedFormStructID"]))
{
	...
	while($rec = $res->FetchRow())
	{
		...
		echo "<td><a href='ControlEvaluationForms.php?FormsRecordsID=".$rec["FormsRecordsID"]."'>".$rec["RelatedRecordID"]."</a></td>";
		echo "<td>".$rec["FormTitle"]."</td>";
