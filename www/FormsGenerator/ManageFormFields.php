	...
	else
	{
		...
		for($c=0; $c<count($list); $c++)
		{
			...
			else
			{
				...
				echo "	<td><a target=_blank href='NewFormLabel.php?Item_FormsStructID=".$FormsStructID."&UpdateID=".$list[$c]->FormsLabelID."'>".$list[$c]->FormsLabelID."</a></td>";
		...
		echo "	<td><a target=_blank href='NewFormField.php?Item_FormsStructID=".$FormsStructID."&UpdateID=".$res[$k]->FormFieldID."'>".$res[$k]->FormFieldID."</a></td>";
...
echo "<tr class=FooterOfTable><td colspan=5 align=center><input type=submit value='حذف'>&nbsp;";
echo "<input type=button value='اضافه کردن فیلد' onclick='javascript: window.open(\"NewFormField.php?Item_FormsStructID=".$FormsStructID."\");'>&nbsp;";
echo "<input type=button value='اضافه کردن برچسب' onclick='javascript: window.open(\"NewFormLabel.php?Item_FormsStructID=".$FormsStructID."\");'>";
echo "</td></tr>";
echo "<tr class=FooterOfTable><td colspan=5 align=center><input type=button value='بازگشت' onclick='javascript: document.location=\"ManageFormsStruct.php\"'></td></tr>";
echo "</table>";
echo "</form>";
?>
<br>
...
<tr>
	<td>
	در زمان اضافه کردن برچسب می توانید برخی مشخصات ارسال کننده فرم را نیز ذکر کنید
	</td>
</tr>
</table>
</html>