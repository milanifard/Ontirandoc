        ...
        if($res->rowCount()>0)
        {
        	...
        	while($rec = $res->fetch())
        	{
        		...
        		if($rec["PermittedValues"]!="")
        			echo str_replace(',',' - ',$rec["PermittedValues"]);
        		echo "<br>";
        	}
        }
...
function ShowClassOrigin($ElementID)
{
	$i=0;
	$mysql = pdodb::getInstance();
	$mysql->Prepare("select * from projectmanagement.RDB2OntoLog 
JOIN mis.MIS_Tables on (RDBEntityID=id)
where OntoEntityID=? and RDBEntityType='TABLE' and OntoEntityType='CLASS'");
...
function ShowDataPropertyOrigin($ElementID)
{
	...
	$mysql = pdodb::getInstance();
	$mysql->Prepare("select * from projectmanagement.RDB2OntoLog 
JOIN mis.MIS_TableFields on (RDBEntityID=id)
where OntoEntityID=? and RDBEntityType='FIELD' and OntoEntityType='DATAPROP'");
	  $res = $mysql->ExecuteStatement(array($ElementID));
	  while($rec = $res->fetch())
	  {
	  	if($i>0)
	  	...
	  	$i++;
	  	//echo $rec["description"];;
	  	echo $rec["TableName"].".".$rec["FieldName"]." (".$rec["description"].")";
	  }
}
